...
export class VehicleLoadManager {
   ...
   static loadCapacity(vehicle) {
      const base = this.rawLoadCapacity(vehicle) ?? this.defaultLoadCapacity(vehicle)
      const bonus = Number(vehicle?.getFlag?.(MODULE_ID, "moduleBonuses")?.loadCapacity) || 0
      return Math.max(0, this._round(base + bonus))
   }
