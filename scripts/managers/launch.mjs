...
export class VehicleLaunchManager {
   ...
   static async toggleLaunched(vehicle, actor = null) {
      ...
      if (!this.isLaunched(vehicle) && !(await this._canStartIgnition(vehicle, actorUuid)))
         return false
      if (game.user.isGM) {
         await this._doToggle(vehicle.id, actorUuid)
   ...
   static async _doToggle(vehicleId, actorUuid = null) {
      ...
      if (existing.length > 0) {
         ...
         SiegeSFXManager.play(vehicle, "stopIgnition")
         ...
         SiegeSFXManager.play(vehicle, "launch")
         ui.notifications.info(
