...
export class SiegeKeybindings {
   static _actionsPanelToggleLocked = false
...
static _resolveVehicle() {
      const targetCrewVehicle = VehicleEntryManager.targetedCrewVehicle(game.user)
      if (targetCrewVehicle) return targetCrewVehicle
      ...
      const crewman =
         controlled[0]?.actor ||
         VehicleEntryManager.singleOwnedSceneActor(game.user) ||
         game.user.character
      if (crewman) {
...
static _userIsCrewOfVehicle(vehicle) {
      return VehicleEntryManager.userOwnedCrewActors(vehicle, game.user).length > 0
   }
   ...
   static async _openActionsPanel() {
      if (this._actionsPanelToggleLocked) return
      this._actionsPanelToggleLocked = true
      let unlocked = false
      const unlock = () => {
         if (unlocked) return
         unlocked = true
         this._actionsPanelToggleLocked = false
