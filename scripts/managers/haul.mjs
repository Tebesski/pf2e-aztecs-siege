...
function haulDebug(...args) {
   const on =
      globalThis.SIEGE_HAUL_DEBUG === true ||
      (game?.settings?.settings?.has?.(`${MODULE_ID}.haulDebug`) &&
         game.settings.get(MODULE_ID, "haulDebug"))
   if (on) console.log("%c[Haul]", "color:#c80", ...args)
}
...
export class HaulManager {
   ...
   static async _recalcHauledTarget(target) {
      ...
      await this.enqueue(haulerId, async () => {
         ...
         haulDebug(
            `recalc target=${target.name} -> wholeBulk=${b.wholeBulk} lightCount=${b.lightCount} (coef=${coef})`,
         )
   ...
   static lightItemCount(actor) {
      ...
      haulDebug(`lightItemCount(${actor?.name}) =`, count)
      return count
   }
   ...
   static inventoryBulk(actor) {
      ...
      if (
         ...
         haulDebug(
            `inventoryBulk(${actor?.name}) via PF2e Bulk: normal=${normal} light=${light} total=${total}`,
         )
         return total
      }
      const val = computed?.value
      if (typeof val === "number") {
         haulDebug(`inventoryBulk(${actor?.name}) via .value=${val}`)
         return val
      }
      ...
      haulDebug(`inventoryBulk(${actor?.name}) via manual sum = ${total}`)
      return total
   }
   ...
   static async haul(hauler, targets, coefficient = 1, elevation = null) {
      ...
      for (const { actor, token } of targetData) {
         ...
         haulDebug(`target=${actor.name}`, {
            isVehicle: breakdown.isVehicle,
            flagBulk: breakdown.flagBulk,
            sizeBulk: breakdown.sizeBulkVal,
            invComputed: breakdown.invComputed,
            invLightCount: breakdown.invLightCount,
            invWhole: breakdown.invWhole,
            coef: safeCoef,
            wholeBulk: breakdown.wholeBulk,
            lightCount: breakdown.lightCount,
         })
      ...
      await this.enqueue(haulerId, async () => {
         ...
         if (target) {
            ...
            for (const { applied, prior } of elevationEntries) {
               ...
               const tokenDoc = token?.document ?? token
               haulDebug(
                  `release elevation: target=${target.name} applied=${applied} prior=${prior} current=${tokenDoc?.elevation}`,
               )
   ...
   static async _onDeleteItem(item, options) {
      ...
      if (item.type === "effect" && item.getFlag(MODULE_ID, "isHauled")) {
         ...
         haulDebug(
            `Hauled effect removed: target=${item.parent?.name} applied=${applied} prior=${prior}`,
         )
      ...
      if (item.type === "effect" && item.getFlag(MODULE_ID, "isHauling")) {
         ...
         haulDebug("Hauling effect removed -> releasing targets:", targetIds)
