...
import { ensureSiegeCSS, getActionsForCrew } from "../macros/helpers.mjs"
   ...
   _renderHTML() {
      ...
      const slots = state.visible
         .map((entry, idx) => {
            ...
            return `<button class="sah-slot ${blocked}" data-entry-id="${entry.id}" draggable="true" data-tooltip="${entry.label}">
