...
const BLOOD1 = "modules/pf2e-aztecs-siege/assets/blood/hud-blood1.png"
const BLOOD2 = "modules/pf2e-aztecs-siege/assets/blood/hud-blood2.png"
const BLOOD3 = "modules/pf2e-aztecs-siege/assets/blood/hud-blood3.png"
const DEAD_ICON = "systems/pf2e/icons/other/dead.svg"
...
static refreshFor(vehicleId) {
   ...
   static async changePositionDialog(vehicle, actor) {
      if (!vehicle || !actor) return
      const existing = this._open.get(vehicle.id)
      const hud = existing || new CrewHUD(vehicle)
      return hud._changePositionDialog(actor, {
         refresh: !!existing,
      })
   }
...
_computeWidth(count) {
   ...
   _gatherCrew() {
      ...
      const showCrewSelector = ownedCrew.length > 1
      const selectedCrew = showCrewSelector
      ...
      for (const actor of actors) {
         ...
         const portraitData = normalizePortraitData(
            perVeh || actor.getFlag(MODULE_ID, "crewPortrait") || null,
            actor.img,
         )
         const isSelf =
            !!game.user.character && game.user.character.id === actor.id
         const owned = actor.testUserPermission(game.user, "OWNER")
         const selectable = showCrewSelector && (game.user.isGM || owned)
         const hpState = this._hpState(actor)
         ...
         cards.push({
            ...
            owned,
            selectable,
            selected: selectable && selectedCrew?.uuid === actor.uuid,
            hpTint: hpState.tint,
            hpBlood: hpState.blood,
            hpPct: hpState.pct,
            defeated: hpState.defeated,
            deadIcon: hpState.deadIcon,
   ...
   _hpState(actor) {
      ...
      if (max <= 0)
         return {
            ...
            blood: "",
            defeated: false,
            deadIcon: globalThis.CONFIG?.controlIcons?.defeated || DEAD_ICON,
         }
      const pct = Math.max(0, Math.min(100, (value / max) * 100))
      const damageRatio = Math.max(0, Math.min(1, 1 - value / max))
      let blood = ""
      if (pct <= 25) blood = "heavy"
      else if (pct <= 50) blood = "medium"
      else if (pct <= 75) blood = "light"
      const tint = damageRatio <= 0 ? 0 : Math.min(0.62, 0.13 + damageRatio * 0.62)
      return {
         pct,
         tint: tint.toFixed(3),
         blood,
...
const isMine = game.user.isGM || card.owned
      ...
      const isEnterable = VehicleEntryManager.isEnterable(this.vehicle)
      const leaveLabel = isEnterable
         ? tKey("CrewHUD.ExitNamed", { name: vehName })
         : tKey("CrewHUD.UnmountNamed", { name: vehName })
      const leaveIcon = isEnterable
      ...
      const actionButtons = isMine
         ? `<div class="crew-card-actions">
               ...
               <button class="crew-card-btn crew-exit-btn" data-actor-uuid="${card.actorUuid}" data-tooltip="${leaveLabel}"><i class="fa-solid ${leaveIcon}"></i></button>
            </div>`
         : `<div class="crew-card-actions">
               <button class="crew-card-btn crew-info-btn" data-actor-uuid="${card.actorUuid}" data-tooltip="${tKey("CrewHUD.Info")}"><i class="fa-solid fa-circle-info"></i></button>
            </div>`
...
const portraitButton = isMine
         ...
         : ""
      const selectedCheckbox = card.selectable
         ? `<input type="checkbox" class="crew-selected-checkbox" data-actor-uuid="${card.actorUuid}" ${card.selected ? "checked" : ""} data-tooltip="${tKey("CrewHUD.SelectedCrewTooltip")}">`
         : ""

const expandedClass = total === 1 ? "crew-card-expanded" : ""
      const damageClass =
         card.hpTint > 0
            ? `crew-card-damaged crew-card-blood-${card.hpBlood || "none"}`
            : ""
      const defeatedClass = card.defeated ? "crew-card-defeated" : ""
      const bloodOverlay =
         card.hpBlood === "heavy"
            ? `<img class="crew-card-blood crew-card-blood-full" src="${BLOOD2}" alt="" loading="eager" decoding="async">`
            : card.hpBlood === "medium"
              ? `<img class="crew-card-blood crew-card-blood-top" src="${BLOOD1}" alt="" loading="eager" decoding="async">
                 <img class="crew-card-blood crew-card-blood-bottom" src="${BLOOD3}" alt="" loading="eager" decoding="async">`
              : card.hpBlood === "light"
                ? `<img class="crew-card-blood crew-card-blood-top" src="${BLOOD1}" alt="" loading="eager" decoding="async">`
                : ""
      const defeatedOverlay = card.defeated
         ? `<img class="crew-card-dead-mark" src="${card.deadIcon}" alt="">`
         : ""

      return `
         <div class="crew-card ${shapeClass} ${expandedClass} ${card.isSelf ? "crew-card-self" : ""} ${card.selected ? "crew-card-selected" : ""} ${damageClass} ${defeatedClass}" data-actor-id="${card.actorId}" style="--crew-damage-alpha:${card.hpTint};">
            <div class="crew-card-inner">
               <div class="crew-card-portrait" data-actor-uuid="${card.actorUuid}">
                  ${
                     card.portraitData.backgroundSrc
                        ? `<img class="crew-card-portrait-bg" src="${card.portraitData.backgroundSrc}" style="${portraitBackgroundImgStyle(card.portraitData)}">`
                        : ""
                  }
                  <img class="crew-card-portrait-fg" src="${card.portraitData.src}" style="${portraitImgStyle(card.portraitData)}">
               </div>
               <div class="crew-card-damage-tint"></div>
               ${bloodOverlay}
               ${defeatedOverlay}
               ...
               ${card.selected ? `<div class="crew-card-selection-glow"></div>` : ""}
            </div>
            ${selectedCheckbox}
            ${portraitButton}
            ${actionButtons}
...
root.querySelectorAll(".crew-card-rank").forEach((el) => {
      ...
      root.querySelectorAll(".crew-exit-btn").forEach((el) => {
         el.addEventListener("click", async (e) => {
            ...
            const actor = await fromUuid(el.dataset.actorUuid)
            if (!actor) return
            if (VehicleEntryManager.isEnterable(this.vehicle)) {
               await VehicleEntryManager.exitVehicle(actor, this.vehicle)
            } else {
               const removed = await SiegeCrewManager.dismountCrewman(
                  actor,
                  this.vehicle,
               )
               if (removed)
...
root.querySelectorAll(".crew-selected-checkbox").forEach((el) => {
         ...
         el.addEventListener("change", async (e) => {
            ...
            root
               .querySelectorAll(".crew-selected-checkbox")
...
async _changePositionDialog(actor, { refresh = true } = {}) {
      const positions = this.vehicle.getFlag(MODULE_ID, "crew") || []
      const eff = actor.itemTypes.effect.find(
         (e) =>
      ...
      if (occupants.length < max) {
         ...
         if (refresh) this.render({ force: false })
         return
      }
