...
export async function mountMacro(crewmanActor = null, siegeActor = null) {
   ...
   if (VehicleEntryManager.isEnterable(siege)) {
      const { enterVehicleMacro } = await import("./enter.mjs")
      return enterVehicleMacro(crewman, siege)
   }

   if (!isSiege(siege))
      return ui.notifications.warn(tKey("Notifications.MustBeSiegeWeapon"))

   const enteredVehicle = crewman.itemTypes.effect.find(
      (e) => e.getFlag(MODULE_ID, "isEntered") && e.getFlag(MODULE_ID, "siegeId"),
   )
   if (enteredVehicle) {
...
async function handleMountClick(e, app, crewman, siege, crewPositions) {
   e.preventDefault()
   e.stopPropagation()
   const chosenPosition = e.currentTarget?.dataset?.position
   ...
   await SiegeCrewManager.syncCrewPositionExtras(crewman, siege, chosenPosition)
   const { VehicleEntryManager } = await import("../managers/entry.mjs")
   if (VehicleEntryManager._userOwnsActor(game.user, crewman))
      await VehicleEntryManager.setSelectedCrewForVehicle(siege, crewman)
...
function buildAllStrikeRules(siege, chosenPosition, actor = null) {
   return buildMountedSiegeRules(siege, chosenPosition, actor)
}
